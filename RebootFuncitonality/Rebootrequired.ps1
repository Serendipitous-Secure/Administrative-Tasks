...
Function Get-InternalPendingReboot
{
	param
	()

	return Test-Path -Path "$env:AllUsersProfile\CentraStage\reboot.flag"
}
...
$Checks = @()
if ($env:ChkCBServicing -eq $true) {$Checks += 'CBServicing'}
if ($env:ChkWindowsUpdate -eq $true) {$Checks += 'WindowsUpdate'} 
if ($env:ChkCCMClientSDK -eq $true) {$Checks += 'CCMClientSDK'}
if ($env:ChkPendComputerRename -eq $true) {$Checks += 'PendComputerRename'}
if ($env:ChkPendFileRename -eq $true) {$Checks += 'PendFileRename'}
if ($env:ChkInternalRebootFlag -eq $true) {$Checks += 'InternalRebootFlag'}

# Build PendingReboot custom object including InternalRebootFlag
$PendingReboot = Get-PendingReboot
$PendingReboot | Add-Member -MemberType NoteProperty -Name InternalRebootFlag -Value $(Get-InternalPendingReboot)

# Build results array for positive checks
$Results = @()
$Checks | ForEach-Object -Process {if ($PendingReboot.$_ -eq $true) {$Results += $_}}

# Reboot if any checks are positive
...
if ( $Results ) {
	...
	if ( $ENV:OverrideCheck -match "False" ) {
		...
		if ( ($ENV:UDF_16 -match "true") -or !($ENV:UDF_17 -match "false") ) {
			...
			Write-Host -Object 'Rebooting due to:'
			$Results
